...
class Sponsor extends StatefulWidget {
  String id ;
  Sponsor({Key? key,required this.id}) : super(key: key);

  @override
  State<Sponsor> createState() => _MyAppStateSponsor();
}
class _MyAppStateSponsor extends State<Sponsor> {
 ...
 Future getsponsorid() async{
    var request = http.MultipartRequest('POST', Uri.parse('https://seahfwebserver.herokuapp.com/controllerlien/GetAll_Sponsor_Conference'));
    request.fields.addAll({
      'idConference': widget.id
    });
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      ...
      if(data1['Reponse']=='Success'){
        List data2 = json.decode(data1['Sponsors']);
        setState(() {
          long=data2.length;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Sponsors',style: TextStyle(color: Colors.white,fontSize: 25,fontWeight: FontWeight.bold)),
        centerTitle: true,
            ...
            itemBuilder: (context, index) {
              return Container(
                ...
                child:ListTile(
                  ...
                  subtitle: Text('email: ${listsupporter[index].emailsupporter}',style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold),),
